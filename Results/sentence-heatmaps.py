...
logit_for_text = logits_per_image[0, 0]  # Select the logit corresponding to the input text
...
assert len(tokens) == len(token_gradients), "Mismatch between token count and gradient scores"
...
for token, grad in zip(tokens, token_gradients):
    ...
    if token.endswith("</w>"):  # Full word detected (marked by </w>)
        if current_word:  # If building a word, append and finalize
            ...
            aggregated_contributions[current_word] = aggregated_contributions.get(current_word, 0) + grad
            current_word = ""  # Reset current word
        else:  # Standalone word
            word = token.replace("</w>", "")
            aggregated_contributions[word] = aggregated_contributions.get(word, 0) + grad
    else:  # Subword token without a word boundary
        current_word += token  # Append to the current word

# Handle any remaining subword
if current_word:
    aggregated_contributions[current_word] = aggregated_contributions.get(current_word, 0)
...
image_resized = image.resize(grads_upscaled.shape[::-1])  # Reverse shape for (width, height)
...
alpha_map = grads_upscaled / grads_upscaled.max()  # Transparency based on attention values
...
base_name = image_path.split("/")[-1].split(".")[0]  # Extract the base name of the image file
text_start = "_".join(text_input.split()[:3])  # Get the first three words of the text input
