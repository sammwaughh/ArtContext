...
def find_matching_paintings(paintings_range, painter_name):
    ...
    if mask_exact.sum() > 0:
        print(
            f"Found {mask_exact.sum()} paintings with exact full match for '{painter_name}'."
        )
    ...
    if mask_first.sum() > 0:
        print(
            f"No full match found. Found {mask_first.sum()} paintings with first name '{first_name}'."
        )
    ...
    if mask_last.sum() > 0:
        print(
            f"No full or first name match found. Found {mask_last.sum()} paintings with last name '{last_name}'."
        )
...
def main():
    ...
    print(
        f"Processed {total_processed} paintings in {elapsed_rounded} seconds. Rate: {rate} seconds per painting."
    )
