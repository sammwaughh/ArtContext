...
def search_unpaywall_by_title(keywords, email, is_oa=None, max_pages=5,
    ...
    sheet_all = wb.active
    sheet_all.title = "All"
    sheet_all.append(["doi", "title", "publisher", "year",
                      "oa_status", "best_oa_location_url", "journal_name"])
...
def download_papers_from_accessible(excel_file, directory_name):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        results = executor.map(lambda r: download_paper(r, directory, sheet_downloaded), rows)
...
painters = [
        ...
        "Johannes Vermeer", "Rembrandt", "Diego Velázquez", "Peter Paul Rubens",
        "El Greco", "Francisco Goya", "Claude Monet", "Pierre-Auguste Renoir",
        "Edgar Degas", "Paul Cézanne", "Vincent van Gogh", "Paul Gauguin",
        "Edouard Manet", "Georges Seurat", "Henri Matisse", "Pablo Picasso",
        "Marc Chagall", "Joan Miró", "Wassily Kandinsky", "Gustav Klimt",
        "Egon Schiele", "Paul Klee", "Kazimir Malevich", "René Magritte",
        "Salvador Dalí", "Frida Kahlo", "Diego Rivera", "Jackson Pollock",
        "Willem de Kooning", "Mark Rothko", "Andy Warhol", "Roy Lichtenstein",
        "Jean-Michel Basquiat", "Lucian Freud", "Francis Bacon", "David Hockney",
        "Georges Braque", "Edward Hopper", "Grant Wood", "Georg Baselitz",
        "Yayoi Kusama", "Mary Cassatt", "Artemisia Gentileschi", "Élisabeth Vigée Le Brun",
        "Sandro Botticelli", "Jan van Eyck", "Pieter Bruegel the Elder", "Albrecht Dürer",
        "Hieronymus Bosch", "Carlo Crivelli", "Giotto di Bondone", "Masaccio",
        "Uccello", "Fra Angelico", "Andrea Mantegna", "Giorgione", "Parmigianino",
        "Bronzino", "Jacopo Tintoretto", "Andrea del Sarto", "Hans Holbein the Younger",
        "Diego Velázquez",  # (some are repeated on purpose—substitute others!)
        "Bartolomé Esteban Murillo", "J. M. W. Turner", "John Constable", "Thomas Gainsborough",
        "Eugène Delacroix", "Jean-François Millet", "Winslow Homer", "Thomas Eakins",
        "Edvard Munch", "Gustave Courbet", "Berthe Morisot", "Amedeo Modigliani",
        "Maurice Utrillo", "Arshile Gorky", "Yves Klein", "Georgia O'Keeffe",
        "Marc Chagall",  # repeated or placeholders—adjust as you wish
        "Henri de Toulouse-Lautrec", "Jean Dubuffet", "Andrew Wyeth", "Keith Haring",
        "Alphonse Mucha", "John Singer Sargent", "Thomas Cole", "Camille Pissarro",
        "Paul Signac", "Claude Lorrain", "Andrea del Verrocchio", "Max Ernst",
        "René Lalique", "Odilon Redon", "Henri Rousseau", "Édouard Vuillard"
    ]

for painter in painters:
    # 1. Extract just the surname (the last 'word')
    surname = painter.split()[-1]

    # 2. Build keywords: e.g., ["Vermeer", "painting"] for "Johannes Vermeer"
    query_keywords = [surname, "art"]
    joined_query = " ".join(query_keywords)  # e.g. "Vermeer painting"

    print(f"[*] Searching for: {joined_query}")
    ...
    search_unpaywall_by_title(joined_query, EMAIL,
                              is_oa=None,
                              max_pages=3,
