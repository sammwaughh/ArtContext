...
def find_and_save_papers_about_bacchus_and_ariadne(
    ...
    for idx, item in enumerate(items, start=1):
        ...
        try:
            unpaywall_resp = requests.get(
                unpaywall_url, params=params_unpaywall, timeout=30
            )
            if unpaywall_resp.status_code == 200:
                ...
                print(
                    f"[WARN] Unpaywall returned status {unpaywall_resp.status_code}, defaulting to non-OA."
                )
                is_oa = False
                pdf_link = ""
        except requests.exceptions.RequestException as e:
    ...
    df = pd.DataFrame(all_papers)
    print(
        f"[DEBUG] DataFrame created with {df.shape[0]} rows and {df.shape[1]} columns."
    )
