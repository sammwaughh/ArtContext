...
def process_artist(artist_dir_name, root_dir, model, processor, device):
    ...
    candidate_embeddings = encode_texts(candidate_contexts_all, model, processor, batch_size=64, device=device)
...
def main():
    ...
    cache_dir = "cache_clip_embeddings"
    os.makedirs(cache_dir, exist_ok=True)
    
    device = "mps" if torch.backends.mps.is_available() else "cpu"
    print(f"Using device: {device}")
    print("Loading CLIP model and processor using Hugging Face...")
    model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
    processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
    model.to(device)
    
    painter_start_index = 5
    painter_end_index = 20  # Inclusive
    ...
    for idx, row in painters_subset.iterrows():
        ...
        cache_file = os.path.join(cache_dir, f"{artist}_clip_embedding_cache.pt")
        ...
        data = process_artist(artist_dir_name, root_dir, model, processor, device)
        if data is None:
