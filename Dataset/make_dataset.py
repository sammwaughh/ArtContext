...
def main():
    ...
    for _, painter_row in painters_subset.iterrows():
        ...
        for idx, row in matching_paintings.iterrows():
            ...
            # Ensure sentence columns are strings.
            sentence1 = str(row["Sentence 1"])
            sentence2 = str(row["Sentence 2"])
            sentence3 = str(row["Sentence 3"])
            # Concatenate the sentences into a single caption.
            caption = " ".join([sentence1, sentence2, sentence3]).strip()
