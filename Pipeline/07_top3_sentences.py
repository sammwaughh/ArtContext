...
def main():
    ...
    # Map header → column number (header row = 1)
    header_map = {}
    for cell in ws[1]:
    ...
    for _, painter_row in painters_subset.iterrows():
        ...
        print("\nProcessing painter: %s (dir: %s)" % (painter_name, artist_dir_name))

        cache_file = os.path.join(cache_dir, f"{artist_dir_name}_cache.pt")
        if not os.path.exists(cache_file):
            print(
                f"No cache file for painter {painter_name} at {cache_file}. Skipping."
            )
            continue
        ...
        for idx, row in matching_paintings.iterrows():
            ...
            for i in tqdm(
                range(0, num_candidates, chunk_size),
            ...
            # Excel rows are 1-indexed; data rows start at idx + 2.
            excel_row = idx + 2
            ws.cell(
                row=excel_row, column=header_map["Sentence 1"], value=best_sentences[0]
            )
            ws.cell(
                row=excel_row, column=header_map["Sentence 2"], value=best_sentences[1]
            )
            ws.cell(
                row=excel_row, column=header_map["Sentence 3"], value=best_sentences[2]
            )
    ...
    print(
        f"Processed {total_processed} paintings in {elapsed_rounded} s "
        f"({rate} s per painting)."
    )
