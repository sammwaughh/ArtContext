...
def create_excel_file(painter_name, works):
    ...
    Write all three DataFrames to separate sheets in a single Excel file (saved in the ExcelFiles directory).
    """
    # If no works were fetched, create an empty DataFrame with the expected columns.
    if works:
        df_main = pd.DataFrame(works)
    else:
        expected_columns = ['title', 'relevance_score', 'id', 'doi', 
...
def process_painter(painter, max_workers):
    ...
    if not works:
        fetch_logger.info(f"No works fetched for {painter}. Skipping further processing.")
...
def main():
    ...
    # Here, start_row=1 refers to the first data row (i.e. row immediately below the headers),
    # and end_row is the last row number (inclusive) to process.
    ...
    for idx, painter_info in enumerate(painters, start=start_row):
        ...
        # process_painter is called with the query_string (for OpenAlex search) and max_workers.
