...
def compute_text_embedding(model, processor, text, device):
    text_inputs = processor(text=text, return_tensors="pt", truncation=True, padding=True)
...
def get_image_attention(model, processor, image, device):
    ...
    attn = outputs.attentions[-1].mean(dim=1).squeeze(0)  # shape: [num_tokens, num_tokens]
...
# --- Visualization ---
def plot_attention_figure(image, tokens, token_scores, attn_map, model_type, timestamp, painting_title, cos_sim, input_text):
    ...
    axs[1].imshow(image)
    im = axs[1].imshow(attn_map, cmap='jet', alpha=0.5, extent=(0, image.size[0], image.size[1], 0))
...
def main():
    ...
    print(f"Selected painting: Creator={creator}, File Name={image_filename}, Title={painting_title}")
    image = load_image(image_dir, image_filename)

    base_cache_file = get_artist_cache_file(painters_xlsx, creator, base_cache_dir, "clip_embedding_cache.pt")
    ft_cache_file = get_artist_cache_file(painters_xlsx, creator, ft_cache_dir, "finetuned_clip_embedding_cache.pt")
    ...
    text_input_base = ("To illustrate, consider Rembrandt's painting known as The Jewish Bride. "
    ...
    # ----- Base CLIP Model -----
    base_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32", attn_implementation="eager")
    base_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
    base_model.config.output_attentions = True
    base_model.to(device)
    tokens_base, token_scores_base = get_text_attention(base_model, base_processor, text_input_base, device)
    attn_map_base = get_image_attention(base_model, base_processor, image, device)
    
    # Compute cosine similarity for Base model.
    base_text_embedding = compute_text_embedding(base_model, base_processor, text_input_base, device)
    base_image_embedding = compute_image_embedding(image, base_model, base_processor, device)
    cos_sim_base = torch.dot(base_text_embedding, base_image_embedding).item()
    print("Cosine similarity (Base):", cos_sim_base)
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plot_attention_figure(image, tokens_base, token_scores_base, attn_map_base, "Base", timestamp, painting_title, cos_sim_base, text_input_base)
    
    # ----- Fine-Tuned CLIP Model -----
    # Load the base CLIP model and then wrap it with the LoRA adapter.
    ft_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32", attn_implementation="eager")
    ...
    ft_model.to(device)
    tokens_ft, token_scores_ft = get_text_attention(ft_model, ft_processor, text_input_ft, device)
    attn_map_ft = get_image_attention(ft_model, ft_processor, image, device)
    
    # Compute cosine similarity for Fine-Tuned model.
    ft_text_embedding = compute_text_embedding(ft_model, ft_processor, text_input_ft, device)
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plot_attention_figure(image, tokens_ft, token_scores_ft, attn_map_ft, "Fine-Tuned", timestamp, painting_title, cos_sim_ft, text_input_ft)
