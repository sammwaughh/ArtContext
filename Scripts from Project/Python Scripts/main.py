...
import nltk
import pandas as pd
from data_loader import get_query_string, load_painters, load_paintings
...
def process_painting(painting, base_dir, painters_df):
    ...
    article_names = [
        re.sub(r"\.pdf$", "", name.strip(), flags=re.IGNORECASE)
    ...
    for article_name in article_names:
        ...
        for file in os.listdir(art_dir):
            ...
            if file.lower().endswith(".md"):
                ...
                windowed_sents = gather_sentences_with_sliding_window(
                    sentences, title, window=WINDOW_SIZE
                )
                candidate_sentences.extend(windowed_sents)
                md_files_read += 1

    print(
        f"Processed {articles_read} articles and {md_files_read} markdown files for painting '{title}'."
    )
    ...
    visual_description = extract_visual_description(
        query_sentence, candidate_sentences, target_word_count=TARGET_WORD_COUNT
    )
