...
def simple_tokenize(text: str) -> List[str]:
    """
    A simple whitespace tokenizer as a fallback if transformers is unavailable.
    Note: This does not reflect CLIP's BPE exactly, but provides a rough token count.
    """
...
def generate_clip_label(row, max_tokens=77):
    """
    Assemble and truncate a CLIP-compatible label for a painting,
    omitting any clauses whose source data is missing.
    """
    parts = []
    
    # Base clause: Title (Year) by Creator.
