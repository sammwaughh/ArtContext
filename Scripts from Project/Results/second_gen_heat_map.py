...
# ── user-tweakable ───────────────────────────────────── #
TEXT_PROMPT = "The Renaissance masters neither created, nor opened, but merely demonstrated the symbolic nature of the myth of female nudity."
...
def saliency_map(model, proc, image, text, device):
    inputs = proc(text=[text], images=image, return_tensors="pt", padding=True).to(
        device
    )
