...
# Read painters.xlsx and collect the “Query String” column for rows in range.
...
for painter in painter_names:
    # Use the painter name to build the input folder & Excel-file paths.
    ...
    # Excel file is expected as  ExcelFiles/<qs>_works.xlsx .
    ...
    df_downloadable["norm_title"] = (
        df_downloadable["Title"].astype(str).apply(normalize_title)
    )
    ...
    for item in pdf_metadata:
        ...
        match = df_downloadable.loc[
            df_downloadable["norm_title"] == norm_work, "Relevance Score"
        ]
    ...
    df_markdown["file size"] = df_markdown["file size"].apply(
